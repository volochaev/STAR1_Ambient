...
**Доступные палитры (21):**
- **Классические:** Sunset Amber, Ocean Blue, Red Moon, Purple Silk
- **Природные:** Glacier, Energize, Polar White, Night Blue
- **Премиум:** Hyacinth, Copper Gold, Rose
- **Премиум люкс:** Diamond White, Silver Mist, Platinum, Amber Gold, Magenta Royal
...
- `FX_GRADIENT_FLOW` / `FX_MB_FLOW_SOFT` - плавный градиент
- `FX_TWIN_WAVE` / `FX_MB_TWO_TONE_WAVE` - двухтонная волна
- `FX_MB_OCEAN_FLOW` - океанский поток (плавное сглаживание)
...
- `FX_DUAL_ZONE` / `FX_MB_DUO_FLOW` - двойная зона
- `FX_MB_SOFT_SOLID` - мягкий статичный цвет
- `FX_MB_SOFT_BREATHE` - мягкое дыхание
...
- **PST_BRIDGE** - плавный переход intro→scene (600мс, quintic smoothstep)
...
```c
#define AMB_ENABLE_AUTO_ROTATE         1     // 1=вкл, 0=выкл
#define AMB_AUTO_ROTATE_INTERVAL_SEC   120u  // интервал смены (секунды)
#define AMB_CROSSFADE_DURATION_MS      3000u // длительность кроссфейда (мс)
```
...
3. Результаты смешиваются с screen blend для предотвращения затемнения
...
- `theme_index` - индекс темы в банке (для extended mode)
...
2. Master читает 0x351 и выбирает тему для банка (с циклической ротацией)
...
- Синхронизируется с яркостью и OEM цветом из CAN (0x351)
...
- По умолчанию: `DEMO_MODE = 1` (включен)
- Для продакшн режима установите: `DEMO_MODE = 0`
...
```c
#define DEMO_MODE  1  // Включен (по умолчанию)
// или
#define DEMO_MODE  0  // Выключен (продакшн режим)
```
...
```c
#define AMB_ENABLE_AUTO_ROTATE         1     // 1=вкл, 0=выкл (по умолчанию: вкл)
#define AMB_AUTO_ROTATE_INTERVAL_SEC   120u  // интервал смены темы (секунды)
#define AMB_CROSSFADE_DURATION_MS      3000u // длительность кроссфейда (мс)
```
...
По умолчанию демо режим включен (`DEMO_MODE = 1`). Для отключения установите в `main.h`:

```c
#define DEMO_MODE  0  // в main.h
```
...
- ✅ **21 цветовая палитра** включая Aurora Borealis, Champagne Gold, Deep Burgundy
- ✅ **Easing функции** для премиальной плавности анимаций
- ✅ **Плавный переход intro→scene** через PST_BRIDGE (quintic smoothstep, 600мс)
- ✅ **Авто-ротация тем** с плавным кроссфейдом (без intro/outro)
...
- **Интеллектуальный старт** (не в DEMO режиме)
  ...
  - Выбор темы на основе extended_mode и сохранённого индекса из Flash
