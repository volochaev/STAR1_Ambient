...
/* Упаковка GRB → PWM-код для WS2812 в dst. */
...
void ws_render(ws2812_t *ws)
{
    ...
    if (!ws->dma_busy) {
        ...
        /* ready_buf становится active_buf */
...
void ws_power_set(uint8_t on)
{
    ...
    /* 74AHCT1G125/244: OE активен низким уровнем.
     * on=1 → включаем передачу → OE = 0.
     * on=0 → тристейт → OE = 1.
     */
