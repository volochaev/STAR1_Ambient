...
{
    if (!dst || !src || !dst->rgb || !src->rgb) return;
    uint16_t n = dst->led_count < src->led_count ? dst->led_count : src->led_count;
    memcpy(dst->rgb, src->rgb, (size_t)n * BYTES_PER_LED);
}
...
void frame_copy_zone(ws2812_t *dst, uint16_t first_dst,
                     ...
                     uint16_t count)
{
    if (!dst || !src || !dst->rgb || !src->rgb || count == 0) return;
    ...
    memmove(&dst->rgb[bd], &src->rgb[bs], (size_t)count * BYTES_PER_LED);
}
...
void frame_mix(ws2812_t *dst,
               ...
               float t)
{
    if (!dst || !a || !b || !dst->rgb || !a->rgb || !b->rgb) return;
    ...
    for (uint32_t i = 0; i < (uint32_t)n * BYTES_PER_LED; ++i) {
        float av = (float)a->rgb[i];
        float bv = (float)b->rgb[i];
        dst->rgb[i] = (uint8_t)(av + (bv - av) * tt + 0.5f);
    }
}
...
void frame_mix_zone(ws2812_t *dst,
                    ...
                    float t)
{
    if (!dst || !a || !b || !dst->rgb || !a->rgb || !b->rgb || count == 0) return;
    ...
    for (uint32_t i = 0; i < n3; ++i) {
        float av = (float)a->rgb[base + i];
        float bv = (float)b->rgb[base + i];
        dst->rgb[base + i] = (uint8_t)(av + (bv - av) * tt + 0.5f);
    }
}
